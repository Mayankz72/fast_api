...
the v2 Contextual Retrieval baseline, see RESOURCES.md) against the current
pipeline, and fails if scores have dropped meaningfully from the checked-in
baseline (tests/fixtures/ci_baseline_scores.json).

Deliberately NOT a re-run of the full 437-example eval: the free-tier Gemini
quota that gated this whole project (see PROGRESS.md) can't support a full
eval on every push, and DeepEval's own judge-model noise (malformed JSON,
occasional low scores on an otherwise-fine answer - also documented in
PROGRESS.md) means a single small run is noisy by nature. Averaging deltas
across all 5 questions x 4 metrics (20 numbers) smooths most of that out
while still catching a real regression (e.g. a chunking change that breaks
retrieval, or a broken prompt) rather than every case in isolation.
...
# regression, not noise. Loose enough to tolerate the judge-model flakiness
# already observed (a handful of examples give ~0.5-point swings on a single
# metric even with no code changes) but still catches a real break, where
# every question in a metric tends to collapse together (e.g. Contextual
