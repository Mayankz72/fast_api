...
Retrieval, see RESOURCES.md): a 1-2 sentence blurb describing where the chunk sits
in its source document, meant to be prepended to the chunk before embedding so an
isolated chunk retains document-level context it would otherwise lose. Reported to
cut failed retrievals ~49% in Anthropic's own benchmark - this is the fix aimed at
this project's measured weak point (Contextual Precision/Recall, see PROGRESS.md).
